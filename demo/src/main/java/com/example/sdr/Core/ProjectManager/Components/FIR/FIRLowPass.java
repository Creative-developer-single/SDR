...
public class FIRLowPass extends BaseComponent {
    ...
    public FIRLowPass(int blockLength, int inputCount, int outputCount, String ID) {
        ...
        this.WindowType = "Hamming"; // 默认窗口类型
    ...
    public void CalculateFilterCoefficients() {
        ...
        for (int i = 1; i <= WindowLength; i++) {
            ...
            filterCoefficients[i - 1].setReal(Math.sin((i - tar) * wc) / (Math.PI * (i - tar)));
        ...
        switch (WindowType) {
            ...
            case "Hamming":
                for (int i = 1; i <= WindowLength; i++) {
                    filterCoefficients[i - 1].setReal(filterCoefficients[i - 1].getReal() * (0.54 - 0.46 * Math.cos(2 * Math.PI * i / (WindowLength))));
                }
                break;
            case "Hanning":
                for (int i = 1; i <= WindowLength; i++) {
                    filterCoefficients[i - 1].setReal(filterCoefficients[i - 1].getReal() * (0.5 - 0.5 * Math.cos(2 * Math.PI * i / (WindowLength))));
                }
                break;
            default:
