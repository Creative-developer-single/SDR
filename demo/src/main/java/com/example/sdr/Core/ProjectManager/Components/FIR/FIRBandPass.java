...
public class FIRBandPass extends BaseComponent{
    ...
    public FIRBandPass(int blockLength, int inputCount, int outputCount, String ID) {
    ...
    public void CalculateFilterCoefficients(){
        ...
        for(int i=1;i<=WindowLength;i++){
            ...
            filterCoefficients[i-1].setReal((Math.sin((i-tar)*wcHigh) - Math.sin((i-tar)*wcLow)) / (Math.PI * (i-tar)));
